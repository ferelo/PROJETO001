...
namespace SiteWebMVC.App_Start
{
    ...
    {
        public static IQueryable<T> Like<T>(this IQueryable<T> source, string nomePropriedade, string valor)
        {
            if (source == null || String.IsNullOrEmpty(nomePropriedade) || String.IsNullOrEmpty(valor))
                return source;

            valor = valor.ToLower();

            var parametro = Expression.Parameter(source.ElementType, String.Empty);
            ...
            var metodoToLower = typeof(string).GetMethod("ToLower", new Type[] { });

            var expressaoToLower = Expression.Call(propriedade, metodoToLower);
            var valorContante = Expression.Constant(valor, typeof(string));
            var expressaoContains = Expression.Call(expressaoToLower, metodoContains, valorContante);

            var predicate = Expression.Lambda<Func<T, bool>>(expressaoContains, parametro);

            var metodoChamadaExpressao = Expression.Call(typeof(Queryable), "Where",
                new[] { source.ElementType },
                source.Expression, Expression.Quote(predicate));

            return source.Provider.CreateQuery<T>(metodoChamadaExpressao);
        }
            ...
            params object[] values)
        {
            ...
            var elementLambda = DynamicExpression.ParseLambda(source.ElementType, null, elementSelector,
                values);
            return source.Provider.CreateQuery(
                Expression.Call(
    ...
    public static class DynamicExpression
    {
        ...
        public static LambdaExpression ParseLambda(Type itType, Type resultType, string expression,
            params object[] values)
        {
            return ParseLambda(new[] { Expression.Parameter(itType, "") }, resultType, expression,
                values);
        }
